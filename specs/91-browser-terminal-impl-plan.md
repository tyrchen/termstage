...
| 9.4 | Add `termstage api` commands for send-text, send-key, run-command wait/capture, and read-screen as CLI wrappers over the HTTP semantic API. | [50](./50-browser-terminal-cli-design.md), [23](./23-local-remote-command-lease-design.md) | 2 days |
...
| 9.6 | Add local backend semantic operations under `termstage session`: `screen`, `send-text`, `send-key`, `exec`/`run-command`, and `scroll`. These commands use the backend adapter directly and do not require a running gateway URL/token. | [50](./50-browser-terminal-cli-design.md), [23](./23-local-remote-command-lease-design.md), [25](./25-rmux-backend-adapter-design.md) | 1 day |
