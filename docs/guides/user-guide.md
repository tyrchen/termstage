...
This guide explains how to run `termstage` as a local browser terminal for live
presentations. It focuses on presenter workflows: starting a session, sharing the
right browser window, preserving state across refreshes, and understanding the local
security boundary.
...
| `--host <addr>` | `127.0.0.1` | You need a loopback address. Non-loopback addresses are rejected. |
...
- It does not provide TLS for LAN or Internet use.
- It does not provide shell sandboxing.
- It does not reduce the privileges of the local user.
- It should not be placed behind tunnels or reverse proxies.
...
| `--host <addr>` | `127.0.0.1` | 绑定 loopback 地址；非 loopback 会直接被拒绝。 |
