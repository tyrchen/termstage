...
`termstage` is a browser terminal presentation tool. By default it starts a
...
stable across browser refreshes. Pod exposure is available only through an explicit
...
Public mode still controls a real shell or tmux session. It validates token, Host,
and WebSocket Origin against the configured public URL, but stronger remote auth,
...
`termstage` 是给现场演示用的浏览器终端。默认模式会在本机启动一个只监听
loopback 的 Rust 服务，生成带访问令牌的本地 URL，然后把浏览器里的终端界面接到
真实的本地 shell 或 tmux 会话上；pod 公网暴露必须显式启用 public mode，并从环境
变量读取访问 token。
...
public mode 仍然是在控制真实 shell 或 tmux 会话。它会按配置的 public URL 校验
